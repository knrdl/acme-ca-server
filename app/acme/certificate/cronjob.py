                ...
                for (
                    ...
                    if not is_expired and settings.mail.warn_before_cert_expires:
                        ...
                        if ok:
                            async with db.transaction() as sql:
                                await sql.exec(
                                    'update certificates set user_informed_cert_will_expire=true where serial_number=$1',
                                    serial_number,
                                )
                    if is_expired and settings.mail.notify_when_cert_expired:
                        ...
                        if ok:
                            async with db.transaction() as sql:
                                await sql.exec(
                                    'update certificates set user_informed_cert_has_expired=true where serial_number=$1',
                                    serial_number,
                                )
            except Exception:
                logger.error(
