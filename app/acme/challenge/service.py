    ...
    for _ in range(3):  # 3x retry
        ...
        try:
            async with httpx.AsyncClient(
                timeout=10, 
                verify=False, http1=True, http2=False, # only http 1.0/1.1 is required, not https
                follow_redirects=False, # todo: redirects are forbidden for now, but RFC states redirects should be supported
