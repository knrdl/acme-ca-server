...
class CaSettings(BaseSettings):
    ...
    encryption_key: Optional[SecretStr]   # encryption of private keys in database
...
class MailSettings(BaseSettings):
    ...
    def sanitize_values(cls, values):
        warn_before_cert_expires = values.get('warn_before_cert_expires', '').lower().strip()
...
if settings.mail.warn_before_cert_expires and settings.ca.enabled and settings.mail.enabled:
    ...
    if settings.mail.warn_before_cert_expires.days > settings.ca.cert_lifetime.days / 2:
        logger.warn('Env var mail_warn_before_cert_expires should be more than half of the cert lifetime')